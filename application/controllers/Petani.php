...
class Petani extends CI_Controller {
	...
	public function daftar($idpoktan)
	{
		...
		if($this->data['user'])
		{
			...
			$this->load->view('back/petani/petani_view', $this->data);
		}
	}

	public function ajax_list($idpoktan)
	{
		$list = $this->person->get_datatables($idpoktan);
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $person) {
			...
			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary btn-fab" href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$person->id_petani."'".')"><i class="material-icons">app_registration</i></a>
				  <a class="btn btn-sm btn-danger btn-fab" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$person->id_petani."'".')"><i class="material-icons">delete_forever</i></a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->person->count_all($idpoktan),
						"recordsFiltered" => $this->person->count_filtered($idpoktan),
						"data" => $data,
				);

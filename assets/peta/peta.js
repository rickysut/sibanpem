...
function set_marker_desa_content(marker_desa, desa, judul, favico_desa, contents)
{
  ...
  if (desa['lng'])
  {
    marker_desa.push(turf.point([desa['lng'], desa['lat']], {name: "provinsi", content: "provinsi", style: L.icon(point_style)}));
  }
  marker_desa.push(turf.polygon(daerah_desa, {content: content, style: style_polygon}));
}

function set_marker_content(marker, daftar_path, judul, nama_wil, contents)
{
  var daftar = JSON.parse(daftar_path);
  var jml = daftar.length;
  var jml_path;
...
function getBaseLayers(peta, access_token)
{
	...
	var defaultLayer = L.tileLayer.provider('OpenStreetMap.Mapnik', {attribution: '<a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="http://simevi.hortikultura.pertanian.go.id">SiMEVI</a>'}).addTo(peta);
  ...
  if (access_token)
  {
    mbGLstr = L.mapboxGL({
      ...
      attribution: '<a href="https://www.mapbox.com/about/maps">© Mapbox</a> | <a href="http://simevi.hortikultura.pertanian.go.id">SiMEVI</a>',
    });
    ...
    mbGLstr = L.tileLayer.provider('OpenStreetMap.Mapnik', {attribution: '<a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="http://simevi.hortikultura.pertanian.go.id">SiMEVI</a>'}).addTo(peta);
    mbGLsat = L.tileLayer.provider('OpenStreetMap.Mapnik', {attribution: '<a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="http://simevi.hortikultura.pertanian.go.id">SiMEVI</a>'}).addTo(peta);
    mbGLstrsat = L.tileLayer.provider('OpenStreetMap.Mapnik', {attribution: '<a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="http://simevi.hortikultura.pertanian.go.id">SiMEVI</a>'}).addTo(peta);
  }

	var baseLayers = {
		'OpenStreetMap': defaultLayer,
		'OpenStreetMap H.O.T.': L.tileLayer.provider('OpenStreetMap.HOT', {attribution: '<a href="https://openstreetmap.org/copyright">© OpenStreetMap</a> | <a href="http://simevi.hortikultura.pertanian.go.id">SiMEVI</a>'}),
...
function poligonWil(marker)
{
    ...
    onEachFeature: function (feature, layer) {
    	if (feature.properties.name == 'nama')
    	{
    		// Beri classname berbeda, supaya bisa gunakan css berbeda
	      layer.bindPopup(feature.properties.content, {'className' : 'nama'});
    	}
    	else
    	{
...
function overlayWil(marker_desa, sebutan_desa)
{
  var poligon_wil_prov = poligonWil(marker_desa);
  //var poligon_wil_dusun = poligonWil(marker_dusun);
  //var poligon_wil_rw = poligonWil(marker_rw);
  //var poligon_wil_rt = poligonWil(marker_rt);
...
function setlegendPetaDesa(legenda, layerpeta, legendData, judul, nama_wil)
{
  ...
  for (var x = 0; x < daftar.length; x++)
  {
    legenda.onAdd = function (layerpeta) {
      var div = L.DomUtil.create('div', 'info legend');
      var labels = ['<strong>Legenda' + ' ' + ' - ' +  ' ' + judul + '</strong>'];
      var categories = [judul + ' ' + legendData['nama_puskesmas']];
      for (var i = 0; i < categories.length; i++)
        {
          div.innerHTML +=
          labels.push(
